...
# 
# df_consumption_vs_output_3_BK %>%
# ggplot(aes(x = household_size, y = yearly_consumpution_kWh, fill=short_code))+
...
```{r}
df_consumption_vs_output_2_BK <- df_consumption_vs_output %>%
  # filter(short_code == "ACT") %>%
  dplyr::group_by(short_code, household_size, swimming_pool_ind) %>%
  dplyr::summarise(mean_annual_consumpution_kWh = yearly_consumpution_kWh,
            mean_min_system_size = min_system_size,
            mean_min_avg_annual_output = min_avg_annual_output)

View(df_consumption_vs_output_2_BK)
```