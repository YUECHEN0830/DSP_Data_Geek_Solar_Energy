...
solar_syd <- df_solar_exposure %>% 
             ...
             mutate(output_kWh = ((daily_exposure * df_dim_solar_panel[8,2] ) / 1000) * (1 / 1.636) * (1 - 0.25))
...
solar_syd_m <- solar_syd %>% 
               group_by(month, year) %>%
               summarise(output_kWh_m = sum(output_kWh))


solar_syd_mean <- solar_syd %>% 
                  group_by(year) %>%
                  summarise(output_kWh_mean = mean(output_kWh))
...
solar_syd_m19 <- solar_syd_m %>% filter(year == 2019)
solar_syd_mean19 <- solar_syd_mean %>% filter(year == 2019)
df_yearly_lga_solar19 <- df_yearly_lga_solar %>% filter(year == 2019)

  ggplot(solar_syd_m19) +
      geom_smooth(aes(x = month, y =  output_kWh_m)) +
      geom_abline(slope = 0, intercept = (df_yearly_lga_solar19$daily_average * 365 / 12), colour = "orange") +
      annotate()
