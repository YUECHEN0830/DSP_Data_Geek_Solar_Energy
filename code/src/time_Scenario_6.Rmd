...
solar_syd <- df_solar_exposure %>% 
             ...
             mutate(output_kWh8 = ((daily_exposure * df_dim_solar_panel[8,2] ) / 1000) * (1 / 1.636) * (1 - 0.25)) %>%     # JKM400N-72H-TV
             mutate(output_kWh13 = ((daily_exposure * df_dim_solar_panel[13,2] ) / 1000) * (1 / 1.636) * (1 - 0.25))       # LG335N1K-V5
...
ggplot(solar_syd) +
  ...
  annotate("text", x=solar_syd$solar_date[350], y = 1.5, label = "Module (JKM400N-72H-TV)", colour ="green") +
...
df_yearly_lga_solar19 <- df_yearly_lga_solar %>% filter(year == 2019)
  ...
  ggplot(solar_syd_m19) +
      ...
      annotate("text", x = 6, y = 200, label = "Average per Customer and Day (AGL)", colour ="orange") +
